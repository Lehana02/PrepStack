...
import { incrementBooksDownloadCount } from "../redux/userSlice";
...
function BooksCard({ book }) {
  ...
  const previewUrl = `https://res.cloudinary.com/${cloudName}/image/upload/${book.path}.pdf`;
  const pdfUrl = `https://res.cloudinary.com/${cloudName}/image/upload/fl_attachment:document/${book.path}.pdf`;
  const thumbnailUrl = `https://res.cloudinary.com/${cloudName}/image/upload/pg_1,c_scale,w_300/${book.path}.png`;
  ...
  const handleDownload = async () => {
    ...
    dispatch(incrementBooksDownloadCount(book._id));
    try {
  ...
  return (
    ...
    >
      ...
      <h3 className="text-xl md:text-2xl font-bold line-clamp-2 mb-2 flex items-center gap-2">
        <FaFileAlt /> {book.topic}
      </h3>

      {/* Extra Info */}
      <div className="flex justify-center gap-20">
        <p className="text-gray-600 text-sm mb-1">
          Resource: <span className="font-semibold">Books</span>
        </p>
        <p className="text-gray-600 text-sm mb-4">
          Downloads: <span className="font-semibold">{book.downloads}</span>
        </p>
      </div>
