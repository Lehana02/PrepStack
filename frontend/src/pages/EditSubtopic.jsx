...
const task=tasks.find(t=>t._id===taskId)
  const subTopic=task?.subTopics?.find((sub) => sub._id === subId);
  const [subName, setSubName] = useState(subTopic?.name||"");
  const [durationDays, setDurationDays] = useState(subTopic?.durationDays||0);
  const [hoursPerDay, setHoursPerDay] = useState(subTopic?.hoursPerDay|| 1);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await axios.patch(
        `${serverUrl}/api/task/edit-sub-topic/${taskId}/subTopics/${subId}`,
        {
          name: subName,
    ...
    <div className="min-h-screen bg-linear-to-br from-[#0f172a] via-[#1e293b] to-[#312e81] flex justify-center items-center p-6">
      ...
      >
        ...
        <button
          ...
          disabled={loading}
        >
          {loading ? <ClipLoader color="white" size={20} /> : <span className="flex justify-center items-center gap-2"><FaEdit /> Edit Subtopic</span>}
        </button>
      </form>
    </div>
  );
